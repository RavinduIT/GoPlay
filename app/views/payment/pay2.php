...
.stepper { display:flex; align-items:center; gap:.75rem; justify-content:center; margin-bottom:1rem; }
.stepper .dot { width:28px; height:28px; border-radius:50%; display:flex; align-items:center; justify-content:center; border:2px solid var(--primary-color); font-weight:700; }
.stepper .dot.active { background:var(--primary-color); color:#fff; }
.stepper .dot.muted { border-color:var(--border-color); color:var(--text-light); }
</style>
...
<div class="container page">
  ...
  <div class="grid">
    ...
    <!-- Step 1: Contact -->
    <div class="card" id="contactStep">
      <div class="card-body">
        <div class="stepper">
          <div class="dot active" id="dot1">1</div><span>Contact</span>
          <i class="fa fa-ellipsis-h" style="color:var(--text-light)"></i>
          <div class="dot muted" id="dot2">2</div><span>Payment</span>
          <i class="fa fa-ellipsis-h" style="color:var(--text-light)"></i>
          <div class="dot muted" id="dot3">3</div><span>Success</span>
        </div>

        <form onsubmit="return goToPayment(event)">
          <div class="form-grid">
            <div><label class="label">Full Name</label>
              <input class="input" type="text" required placeholder="Enter your full name"></div>
            <div><label class="label">Email Address</label>
              <input class="input" type="email" required placeholder="you@email.com"></div>
            <div><label class="label">Phone Number</label>
              <input class="input" type="tel" required placeholder="+1 (555) 123-4567"></div>
            <div><label class="label">Country</label>
              <select class="select" required>
                <option>United States</option><option>United Kingdom</option><option>Sri Lanka</option>
              </select></div>
            <div style="grid-column:1/-1"><label class="label">Street Address</label>
              <input class="input" type="text" required placeholder="123 Main St, Apt 4B"></div>
            <div><label class="label">City</label><input class="input" type="text" required></div>
            <div><label class="label">State/Province</label><input class="input" type="text" required></div>
            <div><label class="label">ZIP/Postal Code</label><input class="input" type="text" required></div>
          </div>

          <div class="actions">
            <button class="btn btn-primary" type="submit">Continue to Payment <i class="fa fa-arrow-right" style="margin-left:.5rem"></i></button>
          </div>
        </form>
      </div>
    </div>
    ...
    <!-- Step 2: Payment -->
    <!-- Step 2: Payment -->
<div class="card" id="paymentStep" style="display:none">
  <div class="card-body">
    <div class="stepper">
      <div class="dot muted">1</div><span>Contact</span>
      <i class="fa fa-ellipsis-h" style="color:var(--text-light)"></i>
      <div class="dot active">2</div><span>Payment</span>
      <i class="fa fa-ellipsis-h" style="color:var(--text-light)"></i>
      <div class="dot muted">3</div><span>Success</span>
    </div>

    <h3>Choose Your Payment Method</h3>
    <p style="color:var(--text-secondary);margin-bottom:1rem">Secure checkout · Your information is protected</p>

    <!-- Cash on Delivery -->
    <div class="payment-option active" onclick="selectPayment(this,'cod')">
      <div><input type="radio" name="method" checked> Cash on Delivery (COD)</div>
      <small style="color:var(--text-secondary)">Pay at delivery</small>
    </div>

    <!-- Credit Card -->
    <div class="payment-option" onclick="selectPayment(this,'card')">
      <div><input type="radio" name="method"> Credit / Debit Card</div>
      <i class="fa fa-credit-card"></i>
    </div>
    <!-- Hidden credit card form -->
    <div id="cardForm" style="display:none; margin-left:2rem; margin-bottom:1rem;">
      <div class="form-grid">
        <div style="grid-column:1/-1">
          <label class="label">Card Number</label>
          <input class="input" type="text" placeholder="4242 4242 4242 4242" maxlength="19">
        </div>
        <div>
        ...
        <div>
          ...
          <input class="input" type="text" placeholder="CVC" maxlength="4">
        </div>
      </div>
    </div>
    ...
    <div class="payment-option" onclick="selectPayment(this,'paypal')">
      <div><input type="radio" name="method"> PayPal</div>
      <i class="fa fa-paypal"></i>
    </div>

    <div style="margin-top:1rem"><input type="checkbox" id="terms"> <label for="terms">I've agreed to the terms and conditions</label></div>

    <div class="actions" style="margin-top:1rem;display:flex;gap:1rem">
      <button class="btn btn-primary" onclick="finishOrder()">Place Order</button>
    ...
    <!-- Step 3: Success -->
    <div class="card" id="successStep" style="display:none;text-align:center">
      <div class="card-body">
        <div class="stepper">
          <div class="dot muted">1</div><span>Contact</span>
          <i class="fa fa-ellipsis-h" style="color:var(--text-light)"></i>
          ...
          <i class="fa fa-ellipsis-h" style="color:var(--text-light)"></i>
          <div class="dot active">3</div><span>Success</span>
        </div>

        <i class="fa fa-check-circle" style="font-size:48px;color:var(--primary-color)"></i>
        <h2>Payment Successful!</h2>
        <p>Thank you for your purchase! Your order has been successfully placed.</p>
        <div style="background:#f8f9fa;padding:1rem;border-radius:10px;margin:1rem 0;text-align:left">
          <strong><i class="fa fa-receipt"></i> Order Status</strong>
          <p>Your order is being processed and you will receive a confirmation email shortly. The shop owner will contact you within 24 hours to arrange delivery.</p>
        </div>
