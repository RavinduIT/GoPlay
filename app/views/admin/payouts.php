 ...
 * Review, approve, and reject shop owner payout requests
 */
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tab-btn { padding: 12px 24px; background: none; border: none; color: #718096; font-size: 15px; font-weight: 600; cursor: pointer; position: relative; transition: color 0.2s; }
            ...
            <div class="admin-header">
                <div class="header-content">
                    <div>
                        ...
                        <p class="page-subtitle">Review and process shop owner payout requests</p>
                    </div>
                </div>
            </div>
            ...
            <div class="content">
                ...
                <div class="tabs-container">
                    <div class="tab-buttons">
                        <button class="tab-btn active" onclick="filterPayouts('pending')" id="tab-pending">
                            <i class="fas fa-clock"></i> Pending <span class="badge" id="pendingBadge">0</span>
                        </button>
                        <button class="tab-btn" onclick="filterPayouts('completed')" id="tab-completed">
                            <i class="fas fa-check"></i> Completed
                        </button>
                        <button class="tab-btn" onclick="filterPayouts('rejected')" id="tab-rejected">
                            <i class="fas fa-times"></i> Rejected
                        </button>
                        <button class="tab-btn" onclick="filterPayouts('all')" id="tab-all">
...
document.addEventListener('DOMContentLoaded', () => loadPayouts('pending'));

async function loadPayouts(status) {
    try {
        const res = await fetch(`${BASE}/api/admin/payouts?status=${status}`);
...
function renderPayouts(payouts) {
    ...
    tbody.innerHTML = payouts.map(p => `
        <tr>
            <td>
                <strong>${esc(p.owner_first_name || '')} ${esc(p.owner_last_name || '')}</strong><br>
                <small style="color: #718096;">${esc(p.shop_name || 'N/A')} &bull; ${esc(p.owner_email || '')}</small>
            </td>
            ...
            <td>
                ${p.status === 'pending' ? `
                    <button class="btn btn-approve btn-sm" onclick="openApprove(${p.id}, '${esc(p.owner_first_name)} ${esc(p.owner_last_name)}', ${p.amount}, '${esc(p.bank_name)}', '${esc(p.account_number)}')">
                        <i class="fas fa-check"></i>
                    </button>
                    <button class="btn btn-reject btn-sm" onclick="openReject(${p.id}, '${esc(p.owner_first_name)} ${esc(p.owner_last_name)}', ${p.amount})">
                        <i class="fas fa-times"></i>
                    </button>
                ` : (p.transaction_reference ? `<small>Ref: ${esc(p.transaction_reference)}</small>` : (p.rejection_reason ? `<small style="color: #e53e3e;">${esc(p.rejection_reason)}</small>` : '-'))}
            </td>
        </tr>
    `).join('');
}
...
function openApprove(id, name, amount, bank, accNo) {
    selectedPayoutId = id;
    document.getElementById('approveDetails').innerHTML = `
        <div class="detail-row"><span class="detail-label">Shop Owner</span><span class="detail-value">${name}</span></div>
...
async function confirmApprove() {
    ...
    try {
        const res = await fetch(`${BASE}/api/admin/payouts/approve`, {
            ...
            body: JSON.stringify({ payout_id: selectedPayoutId, transaction_reference: ref })
        });
        const data = await res.json();
        if (data.success) { closeModal('approveModal'); loadPayouts(currentFilter); } 
...
async function confirmReject() {
    ...
    try {
        const res = await fetch(`${BASE}/api/admin/payouts/reject`, {
            method: 'POST', headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ payout_id: selectedPayoutId, reason: reason })
        });
        const data = await res.json();
        if (data.success) { closeModal('rejectModal'); loadPayouts(currentFilter); }
        else { alert(data.message); }
