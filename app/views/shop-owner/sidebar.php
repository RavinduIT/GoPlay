<?php
/**
 * Shop Owner Sidebar - Dark Theme (matching Admin Sidebar exactly)
 */
$_base = defined('BASE_URL') ? BASE_URL : '';
$currentPath = $_SERVER['REQUEST_URI'] ?? '';
$activePage = '';
if (strpos($currentPath, '/shop-owner/products') !== false) $activePage = 'products';
elseif (strpos($currentPath, '/shop-owner/orders') !== false) $activePage = 'orders';
elseif (strpos($currentPath, '/shop-owner/inventory') !== false) $activePage = 'inventory';
elseif (strpos($currentPath, '/shop-owner/reviews') !== false) $activePage = 'reviews';
elseif (strpos($currentPath, '/shop-owner/earnings') !== false) $activePage = 'earnings';
elseif (strpos($currentPath, '/shop-owner/profile') !== false) $activePage = 'profile';
elseif (strpos($currentPath, '/shop-owner/dashboard') !== false) $activePage = 'dashboard';

$menuItems = [
    ['id'=>'dashboard','url'=>$_base.'/shop-owner/dashboard','icon'=>'fas fa-home','label'=>'Dashboard','badge'=>null],
    ['id'=>'products','url'=>$_base.'/shop-owner/products','icon'=>'fas fa-box','label'=>'Products','badge'=>null],
    ['id'=>'orders','url'=>$_base.'/shop-owner/orders','icon'=>'fas fa-shopping-cart','label'=>'Orders','badge'=>null],
    ['id'=>'inventory','url'=>$_base.'/shop-owner/inventory','icon'=>'fas fa-warehouse','label'=>'Inventory','badge'=>null],
    ['id'=>'reviews','url'=>$_base.'/shop-owner/reviews','icon'=>'fas fa-star','label'=>'Reviews','badge'=>null],
    ['id'=>'earnings','url'=>$_base.'/shop-owner/earnings','icon'=>'fas fa-wallet','label'=>'Earnings & Payouts','badge'=>null],
    'divider',
    ['id'=>'profile','url'=>$_base.'/shop-owner/profile','icon'=>'fas fa-user','label'=>'Profile','badge'=>null],
    ...
    <div class="sidebar-footer">
        <div class="user-info">
            ...
            <div class="user-details">
                <span class="user-name"><?= htmlspecialchars($_SESSION['user_name'] ?? $_SESSION['first_name'] ?? 'Shop Owner') ?></span>
...
<style>
/* === DARK THEME SIDEBAR (exact match of admin-sidebar.php) === */
...
.nav-badge{background:#3b82f6;color:#fff;padding:2px 8px;border-radius:12px;font-size:11px;font-weight:600;min-width:20px;text-align:center;opacity:1;transition:opacity .2s ease}
...
@media(max-width:768px){
    ...
    .admin-main,.dashboard-main{margin-left:0!important}
}
