...
include __DIR__ . '/../ground-owner/layout-head.php';
?>
        ...
        <div class="go-page-header">
            <div class="go-header-left">
                ...
                <div>
                    ...
                    <p class="go-page-subtitle">Admin-issued platform commission notices for your completed coaching sessions</p>
                </div>
            </div>
        </div>
            ...
            <div class="ci-card">
                ...
                <div style="overflow-x:auto;padding:0 20px 20px">
                    ...
                    <table id="ciTable" style="display:none;width:100%;border-collapse:collapse;font-size:13px">
                        <thead>
                            <tr>
                                <th>Invoice #</th><th>Period</th><th>Total Session Earnings</th>
...
async function loadInvoices(){
    ...
    try{
        const r = await fetch(BASE+'/api/coach/commission/invoices');
        ...
        document.getElementById('ciTbody').innerHTML=rows.map(inv=>{
            ...
            return '<tr>'+
                '<td><strong style="color:#2563eb">'+esc(inv.invoice_number)+'</strong></td>'+
        ...
        console.error(e);
        document.getElementById('ciLoading').innerHTML='<i class="fas fa-exclamation-circle"></i> Failed to load';
    }
}
...
window.viewInvoice=async function(id){
    ...
    try{
        const r=await fetch(BASE+'/api/coach/commission/invoices/'+id);
        const d=await r.json();
        if(!d.success){ document.getElementById('ciDetailContent').innerHTML='<p>Failed to load.</p>'; return; }
        const inv=d.invoice;
        const items=(inv.items||[]).map(it=>
            '<div class="ci-row">'+
            '<span>'+(esc(it.facility_name)||'—')+' &nbsp;<span style="color:#94a3b8;font-size:12px">'+fmtDate(it.earning_date)+'</span></span>'+
        ...
        document.getElementById('ciDetailContent').innerHTML=
            ...
            '<div><span style="color:#64748b;font-size:12px">Invoice #</span><br><strong>'+esc(inv.invoice_number)+'</strong></div>'+
            ...
            items+
            '<div class="ci-row" style="font-weight:700"><span>Total Session Earnings</span><span>'+lkr(inv.total_earnings)+'</span></div>'+
            '<div class="ci-row" style="color:#d97706;font-weight:700"><span>Commission (10%)</span><span>'+lkr(inv.commission_amount)+'</span></div>'+
            '</div>'+
            (inv.notes?'<p style="font-size:13px;color:#374151"><strong>Notes:</strong> '+esc(inv.notes)+'</p>':'');
    }catch(e){
        console.error(e);
    }
};
...
<?php include __DIR__ . '/../ground-owner/layout-foot.php'; ?>
